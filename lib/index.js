 ...
 */
import { DAY_OPTIONS, isRecord, normalizeConfig } from "./core/core.js";
import { memoize } from "./host/memo.js";
import { isTrustedApiRequest, readJsonBody, writeJson, writeText } from "./host/wire.js";
import { ConfigStore, defaultStorageDir } from "./services/config-store.js";
import { scanUsage } from "./services/usage.js";
import { buildOverview, buildUsage } from "./services/monitor.js";
import { PlatformMonthCache, buildPlatformSpend, platformPlaceholder } from "./services/platform.js";
import { AlertNotifier } from "./services/alerts.js";
...
const CONTRACT_SERVICES = ['webServer', 'httpServer', 'credentials', 'sessionPersistence', 'agentDefaultModel'];
...
export function apply(ctx, rawConfig) {
    ...
    const readWeb = () => (ctx.get('webServer') ?? ctx.get('httpServer'));
    const readCredentials = () => ctx.get('credentials');
    const readPersistence = () => ctx.get('sessionPersistence');
    const readAgentDefaultModel = () => ctx.get('agentDefaultModel');
    // ---- contract self-check ------------------------------------------------
    ...
    const contractLine = (reason) => {
        const present = [];
        const missing = [];
        for (const key of CONTRACT_SERVICES)
            (readService(key) === undefined ? missing : present).push(key);
        return `[dsh-usage-monitor] contract check (${reason}) present=[${present.join(' ')}] missing=[${missing.join(' ')}]`;
    };
    const warnIfNoWeb = () => {
        if (readService('webServer') !== undefined || readService('httpServer') !== undefined)
            return;
        console.error('[dsh-usage-monitor] webServer/httpServer 均未出现：本地 JSON API 无法挂载，面板会显示「尚未加载到数据」。' +
    ...
    const usageMemo = memoize(USAGE_TTL_MS, scan);
    const usageGet = (force) => (force ? usageMemo.refresh() : usageMemo.get());
    // DeepSeek 平台实际消费：独立 memo + 非阻塞快照。首次取数在后台进行，
    // /overview 立刻返回 'loading' 占位，浏览器下一次轮询就能看到真实数据。
    const platformMemo = memoize(PLATFORM_TTL_MS, () => {
        const cfg = store.get();
    ...
    const platformGet = (force) => {
        const settle = (value) => {
            platformSnapshot = value;
            return value;
        };
        if (force)
            return platformMemo.refresh().then(settle);
        const hit = platformMemo.peek();
        if (hit !== undefined && Date.now() - hit.setAt < PLATFORM_TTL_MS)
            return Promise.resolve(settle(hit.value));
    ...
    const overviewMemo = memoize(BALANCE_TTL_MS, () => buildOverview(store.get(), { credentials: readCredentials(), agentDefaultModel: readAgentDefaultModel() }, () => usageMemo.get(), Date.now()));
    const overviewGet = async (force) => {
    ...
    const disposeEvents = ctx.on('session/event', (...args) => {
        const event = args[1];
    ...
    if (typeof disposeEvents === 'function')
        ctx.effect(disposeEvents, 'dsh-usage-monitor: cache bust');
    ...
    const serviceNameOf = (args) => {
        for (const arg of args)
            if (typeof arg === 'string' && arg !== '')
                return arg;
        return '';
    };
    const onServiceReady = (name) => {
    ...
    const disposeServiceWait = ctx.on('internal/service', (...args) => onServiceReady(serviceNameOf(args)), { global: true });
    if (typeof disposeServiceWait === 'function')
        ctx.effect(disposeServiceWait, 'dsh-usage-monitor: late route mount');
    let mountAttempts = 0;
    let mountTimer;
    const retryMount = () => {
        if (routesMounted || mountAttempts >= 40)
            return;
        mountAttempts += 1;
...
async function route(store, usageGet, overviewGet, platformGet, clearCaches, req, res) {
...
function usageParams(cfg, search) {
    const raw = Number(search.get('days'));
    const days = Number.isFinite(raw) && DAY_OPTIONS.includes(Math.round(raw)) ? Math.round(raw) : 30;
    return {
        days,
...
function exportCsv(group, data) {
    const pairs = group === 'session'
        ? [
            ['date', '日期'],
            ['title', '会话'],
            ['sessionId', '会话ID'],
