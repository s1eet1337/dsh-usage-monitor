import { PROVIDERS, providerMeta, canonicalProvider, evaluateAlert, budgetFor, rollDaily, rollRows, rollSessionRows, dayKeyOfMs, totalsOf } from "../core/core.js";
import { getAdapter } from "../adapters/index.js";
import { resolveProviderKey } from "./keys.js";
function todayCostFor(records, provider, todayKey) {
    let cost = null;
    for (const r of records) {
        if (canonicalProvider(r.provider) !== provider)
            continue;
        if (dayKeyOfMs(r.at) !== todayKey)
            continue;
        if (r.cost !== null)
            cost = (cost ?? 0) + r.cost;
    }
...
function buildProviderView(meta, now) {
    return {
        provider: meta.id,
        label: meta.label,
        keyConfigured: false,
...
export async function buildOverview(config, deps, scan, nowMs = Date.now(), platform = null) {
    ...
    for (const r of scanResult.records) {
        if (!dayKeyOfMs(r.at).startsWith(monthPrefix))
            continue;
        if (r.cost === null)
            continue;
    ...
    const views = await Promise.all(PROVIDERS.map(async (meta) => {
        ...
        if (typeof cfg?.balanceUrl === 'string' && cfg.balanceUrl !== '')
            opts.baseUrl = cfg.balanceUrl;
        try {
...
export async function buildUsage(scan, params, nowMs = Date.now()) {
    const scanResult = await scan();
...
export function balanceMoney(view) {
    if (view.balance?.supported !== true || !Number.isFinite(view.balance.amount))
        return null;
