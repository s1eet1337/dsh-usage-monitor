 ...
 * empty ledger).
 */
async function readSessionEvents(persistence, sid) {
...
const isMissingSession = (err) => {
    const name = err?.name;
    return typeof name === 'string' && name.includes('NotFound');
};
import { errorMessage } from "../core/core.js";
import { estimateCostUsd } from "../core/pricing.js";
import { canonicalProvider } from "../core/core.js";
function toCount(v) {
 ...
 */
export async function scanUsage(persistence, retentionDays, pricingOverrides, nowMs = Date.now()) {
    const coverage = {
        listedSessions: 0,
        scannedSessions: 0,
        usageRecords: 0,
    ...
    for (const sid of sessionIds) {
        ...
        try {
            ...
            if (isMissingSession(err))
                continue; // artifact removed between list and open
            coverage.failedSessions += 1;
        ...
        for (const ev of events) {
            ...
            if (coverage.earliestAt === null || time < coverage.earliestAt)
                coverage.earliestAt = time;
            if (coverage.latestAt === null || time > coverage.latestAt)
                coverage.latestAt = time;
