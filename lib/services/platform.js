...
import { dayKeyOfMs, isRecord, pad2, pickNumber } from "../core/core.js";
import { fetchMonth, fetchUserSummary, monthPointOf } from "../adapters/deepseek-platform.js";
...
export class PlatformMonthCache {
    ...
    constructor(dir) {
        ...
        try {
            ...
            if (!isRecord(raw))
                return;
            if (typeof raw.startMonth === 'string' && /^\d{4}-\d{2}$/.test(raw.startMonth))
                this.startMonth = raw.startMonth;
            if (isRecord(raw.months)) {
                for (const [key, value] of Object.entries(raw.months)) {
                    if (!/^\d{4}-\d{2}$/.test(key) || !isRecord(value))
                        continue;
    ...
    setStart(month) {
        ...
        if (this.startMonth !== null && this.startMonth <= month)
            return;
...
export async function buildPlatformSpend(args) {
    const nowMs = args.nowMs ?? Date.now();
    const currentKey = monthKeyOf(nowMs);
    const spend = baseSpend(nowMs, currentKey);
    const fail = (status, message) => ({ ...spend, status, error: message });
    if (!args.enabled)
        return fail('disabled', '平台同步已关闭');
    const token = args.token?.trim() ?? '';
    if (token === '')
        return fail('missing-token', '未配置 DeepSeek 平台 userToken');
    const options = args.options ?? {};
    const cache = args.cache;
    // ---- wallet + platform-reported monthly totals ----
    const summary = await fetchUserSummary(token, options);
    if (!summary.ok)
        return fail(summary.code, summary.message);
    const wallet = summary.data === null
        ? null
        : { normal: summary.data.normal, bonus: summary.data.bonus, currency: summary.data.currency };
    spend.wallet = wallet;
    if (wallet !== null)
        spend.currency = wallet.currency;
    ...
    if (!current.ok)
        return { ...fail(current.code, current.message), wallet };
    ...
    for (let i = 0; i < Math.max(0, args.historyMonths - 1); i++) {
        ...
        if (cache.knownStart !== null && cursor < cache.knownStart)
            break;
        pending.push(cursor);
    ...
    const note = (month, entry, fetched) => {
    ...
    for (let index = 0; index < pending.length && !stopped; index += BATCH) {
        ...
        const results = await Promise.all(batch.map(async (month) => {
            ...
            if (month === prevKey && previous.ok)
                return { month, entry: { tokens: previous.data.tokens, cost: previous.data.cost }, fetched: false, failed: false };
            const cached = cache.get(month);
            if (cached !== undefined)
                return { month, entry: cached, fetched: false, failed: false };
            const res = await fetchMonth(token, month, options);
            if (!res.ok)
                return { month, entry: { tokens: 0, cost: 0 }, fetched: false, failed: true };
