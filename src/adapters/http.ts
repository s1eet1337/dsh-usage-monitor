...
export async function httpRequest(args: HttpArgs): Promise<HttpResult> {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      throw aborted
        ...
        : new Error(`请求失败：${(err as { message?: string })?.message ?? String(err)}`)
    } finally {
...
export function httpError(code: 'auth' | 'http' | 'network', statusOrMessage: number | string): string {
