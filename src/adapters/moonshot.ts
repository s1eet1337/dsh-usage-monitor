...
export function createMoonshotAdapter(): ProviderAdapter {
  return {
    id: 'moonshot',
 ...
 */
export async function fetchKimiCodingUsage(apiKey: string, options: AdapterOptions = {}): Promise<RemoteUsageOutcome> {
  try {
    const res = await httpRequest({
    ...
    if (!res.ok) return { ok: false, code: res.status === 401 || res.status === 403 ? 'auth' : 'http', message: `用量接口返回错误（HTTP ${res.status}）` }
