...
export type BalanceOutcome =
  | { ok: true; data: BalancePayload }
  | { ok: false; code: 'unsupported' | 'missing-key' | 'auth' | 'http' | 'network' | 'parse'; message: string }
...
export type RemoteUsageOutcome =
  | { ok: true; data: RemoteUsagePayload | null }
  | { ok: false; code: 'unsupported' | 'missing-key' | 'auth' | 'http' | 'network' | 'parse'; message: string }
...
export const unsupportedOutcome = (id: ProviderId, docs: string): BalanceOutcome => ({
  ok: false,
  code: 'unsupported',
