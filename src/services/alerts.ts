...
import type { AlertLevel, MonitorConfig, ProviderView } from '../core/core.ts'
...
export class AlertNotifier {
  private readonly last = new Map<string, AlertLevel>()
  ...
  process(providers: readonly ProviderView[]): void {
    ...
    for (const view of providers) {
      ...
      for (const hook of cfg.webhooks) {
        if (hook.enabled === false || hook.url === '') continue
  ...
  private async dispatch(url: string, secret: string | undefined, payload: AlertPayload): Promise<void> {
...
const cooldownOf = (configured: number | undefined): number =>
...
interface AlertPayload {
  ...
  budget: { amount: number | null; currency: string; warnPct: number } | null
  remainingPct: number | null
  message: string
}

function buildAlertPayload(view: ProviderView, level: AlertLevel, at: number): AlertPayload {
