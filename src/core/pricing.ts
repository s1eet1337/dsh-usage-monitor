...
const EXACT_ONLY = new Set<string>(['o1', 'o3', 'o4-mini', 'o3-mini', 'o1-mini'])
...
const sameProvider = (a: string, b: string): boolean =>
  a === b || a.startsWith(`${b}-`) || b.startsWith(`${a}-`)

export function lookupRate(provider: string, model: string, overrides: readonly PricingOverride[] = []): PriceRate | undefined {
  ...
  for (const [prov, matcher, rate] of PRICE_TABLE) {
    if (!sameProvider(prov, provider)) continue
    const m = matcher.toLowerCase()
    if (EXACT_ONLY.has(matcher)) {
      if (lower === m) return rate
